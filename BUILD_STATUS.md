...
- ✅ Mac runner INTEGRATED (`mac_runner.py`): WSL↔Mac over Tailscale SSH proven (Xcode 26.5, simctl live, remote exec works). iOS simulator E2E LIVE ✅ (booted iPhone 17 Pro on the Mac, screenshotted home screen, pulled to WSL, stored in object store — proven). Android E2E LIVE ✅ too: emulator (Pixel/API-35, arm64) booted on the Mac, screenshotted, pulled to WSL, stored. WSL builds APKs; Mac runs both iOS sims + Android emulator. The OS is a full cross-platform mobile runner.
