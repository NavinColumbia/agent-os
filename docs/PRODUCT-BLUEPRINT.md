...
6. **[06 — Architecture hardening & publishing](blueprint/RESEARCH-architecture-publish.md)** — production-grade inter-agent comms (idempotency/dead-letter/backpressure/saga), AI-aware watchdog/observability/self-healing, multi-tenant isolation (microVM), and the concrete engineering + compliance steps to ship across App Store/Play/web. 62 cited sources, second research-fleet dogfood. *(Several findings are now **implemented** — see the load-bearing constraints below.)*
...
## ⚠️ Load-bearing constraints (from the deep-research dogfood)
*(Two parallel research fleets, 8-way fan-out each, ~150 cited sources combined: [launch playbook](blueprint/RESEARCH-launch-onboarding.md) + [architecture & publishing](blueprint/RESEARCH-architecture-publish.md).)*
1. **Neither store lets you publish your users' generated apps centrally.** Apple Guideline 4.2.6 rejects binaries from app-generation services, Google Play reaches the same end via anti-spam/white-label rules — **each end user publishes under their *own* developer account** (or steer them to **web/PWA**, no gatekeeper). The product *generates the artifact + walks each user through publishing under their own account*; it is not a central publisher.
2. **Apple 2.5.2 + Google's AccessibilityService prohibition are the hardest publish blockers** for an autonomous developer tool (both enforced with high-profile rejections in early 2026): an app may not download/execute new code that changes its purpose, and may not drive other apps via the accessibility API. ⇒ the **agent runtime/execution must live server-side** (which is already our model — the phone/web app is a control surface), not as on-device code execution.
3. **Mandatory, named AI-consent** before any user data reaches a third-party model (Apple 5.1.2(i) eff. 2025-11-13; Google Play AI policy; EU AI Act Art. 50). ✅ *built:* `consent.py` gate.
4. **EU AI Act (full enforcement 2026-08-02) requires runtime human oversight** — the operator must be able to **stop/override agents at runtime**, plus Art. 50 disclosure + an open-loop, human-in-the-loop architecture. ✅ *built:* `killswitch.py` (global/scoped halt that `factory.agent` honors at every spawn).
5. **Exactly-once delivery is impossible across two systems** — the production standard is **at-least-once + idempotent consumers** (outbox/inbox dedup, dead-letter, backpressure). The MAST study (2025) found 41–87% failure rates in *unstructured* multi-agent networks; bounded queues + semaphores + circuit breakers are the minimum. ✅ *partially built:* durable task queue (retry/dead-letter/lease reclaim) + `_AGENT_SEM` + `appguard`; **next:** inbox-dedup `commbus` + saga rollback.
6. **Untrusted agent code needs microVM isolation** (Firecracker/Kata) — containers are not a security boundary. 🟡 sandboxed today (`srt`, network-denied); microVM is the hardening step.
