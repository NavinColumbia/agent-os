...
const VIEWS={
 ...
 agents:async()=>{const d=await get('/api/agents');d.agents=d.agents||[];const roles=(d.roles||['research-growth']);const sys=d.system||[];
  ...
  try{let live=null;try{live=await get('/api/org/live')}catch(e){}
   ...
   if(flat.length){h+='<div class=card><h2>Live team · currently hired</h2><p class=muted style=margin:0_0_10px>The agents your Controller has hired and put to work right now.</p>'+flat.map(n=>{const st=(n.status||'').toLowerCase();const stc=st==='working'?'accent':(st==='done'?'ok':(st==='blocked'?'bad':''));return `<div class=item><div class="row spread"><span><b>${esc(n.name)}</b> ${pill(n.role)} ${n.status?pill(n.status,stc):''}</span>${n.actor_id?`<button onclick="msgAgent(${n.actor_id})" title="Send this agent a direct message">Message</button>`:''}</div>${n.assignment?('<div class=muted>'+esc(n.assignment)+'</div>'):''}<div id="msg_${n.actor_id}"></div></div>`;}).join('')+'</div>';}
  }catch(e){}
 ...
 settings:async()=>{const d=await get('/api/settings');const c=d.ai_consent||{};const pr=d.profile||{};$('#view').innerHTML=`<h1>Settings</h1><p class=sub>Profile, AI consent, keys, notifications.</p>
  ...
  <div class=card><h2>BYO API key</h2><div class=row>${d.byo_key_set?pill('key on file','ok'):pill('no key','warn')}</div><div style=margin-top:8px><input id=bkey aria-label="API key" placeholder="sk-… (stored encrypted)"><button class=pri style=margin-top:6px onclick=saveKey()>save key</button></div><div id=bknote class=muted style=margin-top:8px></div></div>
  ...
  <div class=card><h2>Your data</h2><div class=row><button onclick=acctExport()>Export my data</button><button onclick=acctDelete() style="border-color:var(--r);color:var(--r)">Delete my account</button></div><div id=acctnote class=muted style=margin-top:8px></div></div>
  <div class=card><h2>Audit trail integrity</h2><p class=muted>Every action your AI company takes is written to a tamper-evident, per-company cryptographic hash chain. Verify your complete trail end-to-end — no need to trust us:</p><div class=row><button onclick=verifyAudit()>Verify my audit trail</button></div><div id=auditnote class=muted style=margin-top:8px></div></div>`;PREFS=d.notification_prefs;},
...
async function saveKey(){const v=(($('#bkey')||{}).value||'').trim();const note=$('#bknote');
 if(!v){if(note)note.textContent='Enter your API key first.';const f=$('#bkey');if(f)f.focus();return}   // empty click no longer silently no-ops
 if(note)note.textContent='saving…';const r=await post('/api/byok',{key:v});if(r&&r.error){if(note)note.textContent='✗ '+r.error;else alert('Could not save key: '+r.error);return}
 if(note)note.textContent='✓ Key saved.';go('settings');}
...
async function verifyAudit(){const n=$('#auditnote');if(n)n.textContent='verifying the cryptographic chain…';const r=await get('/api/audit/verify');if(!n)return;n.textContent=(r&&r.intact)?'✓ Cryptographically INTACT — your complete audit trail is unbroken and untampered.':('✗ Integrity check FAILED: '+((r&&r.reason)||'chain broken')+' — please contact support.');n.style.color=(r&&r.intact)?'var(--g)':'var(--r)';}
