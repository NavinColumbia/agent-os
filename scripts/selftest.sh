...
set -u
# Every command launched by this harness—including retry/approval paths—must stay process/model-free unless
# its individual gate explicitly supplies a deterministic local fixture. Child shells inherit this marker.
export AOS_SELFTEST=1
cd "$(dirname "$0")/.."
PY=.venv/bin/python
...
ckp "console QA-fix regression guard"        "$PY scripts/test_console_fixes.py | grep -q '^PASS: all QA-found console fixes'"
...
ckp "north-star scorecard harness sound"     "$PY scripts/northstar_accept.py selftest | grep -q 'harness is sound'"
...
ckp "skills consumed into brief (A4)"     "$PY -c \"import sys;sys.path.insert(0,'scripts');import skills,factory;b=skills.brief_for('backend-engineer');assert 'YOUR CAPABILITIES' in b;print('PASS')\" | grep -q PASS"
...
ckp "cross-process claude gate (global concurrency)" "$PY scripts/claude_gate.py selftest | grep -q 'claude_gate selftest: PASS'"
ckp "agent calls are globally gated (factory wires the gate)"  "$PY -c \"from pathlib import Path;s=Path('scripts/factory.py').read_text();assert s.count('claude_gate.agent_slot') >= 2 and 'claude_gate.process_holder' in s;print('PASS')\" | grep -q PASS"
...
ckp "js test runner resists filename injection"      "$PY -c \"import sys,tempfile;sys.path.insert(0,'scripts');import factory;from pathlib import Path;d=Path(tempfile.mkdtemp());m=d/'AOS_PWNED';(d/(chr(97)+chr(39)+'\$(touch AOS_PWNED)'+chr(39)+'b.test.js')).write_text('process.exit(0)');factory.run_js_tests(str(d));print('VULN' if m.exists() else 'PASS')\" | grep -q PASS"
