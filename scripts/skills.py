...
CATALOG = [
    ...
    ("founder-digest", "growth", "Daily/weekly digest to phone: portfolio + profitability + decisions + risks + next steps", ["controller", "marketing-growth"], ["digest", "ntfy"], R, TO),
    ("risk-and-decision-register", "governance", "Risk register (what could hurt the business) + decision queue (what awaits the human); agent-queryable", ["controller", "incident-commander"], ["risk", "osq"], R, TO),
    ...
    ("agent-dispatcher", "governance", "Activation loop: polls the task queue + WAKES idle agents to process queued work + replies to requester (so messages don't sit unread)", ["controller"], ["dispatcher", "orchestrate"], R, TO),
    ("agent-cli-behavior-probe", "qa", "Empirically behavior-test an agent CLI (Codex): responsiveness, hang-on-block, file-write, token capture, clean exit", ["qa-security", "ml-engineer"], ["codex_probe", "codex"], R, TO),
