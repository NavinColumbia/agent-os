...
def sweep(dry=False):
    ...
    if stuck and not dry:
        with psycopg.connect(DB) as c, c.cursor() as cur:
            # count the lost lease as an attempt + apply backoff, so a poison task still dead-letters
            cur.execute("""UPDATE tasks SET status='pending', attempts=COALESCE(attempts,0)+1, locked_at=NULL,
    ...
    return {"reclaimed": reclaimed, "reclaimable": stuck, "dead_letter_depth": dead}
...
def _selftest():
    ...
    with psycopg.connect(DB) as c, c.cursor() as cur:
        ...
        cur.execute("""INSERT INTO tasks (assignee,title,status,locked_at)
                       VALUES (%s,%s,'active', now()) RETURNING id""", (ag, f"fresh {suf}"))
