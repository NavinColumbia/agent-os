...
def check():
    ...
    with psycopg.connect(DB) as c, c.cursor() as cur:
        ...
        for comp, age in cur.fetchall():
            if age and age > 600:   # >10 min since last beat
...
def tick():
    ...
    paged = []
    with psycopg.connect(DB) as c, c.cursor() as cur:
        cur.execute("SELECT signature, last_sent FROM watchdog_alerts")
        known = {s: ls for s, ls in cur.fetchall()}
        for i in issues:
            ls = known.get(i["sig"])
            fresh = ls is None
            cooled = ls is not None and (time.time() - ls.timestamp()) > COOLDOWN_S
            cur.execute("""INSERT INTO watchdog_alerts (signature, level, last_sent)
                        ...
                        (i["sig"], i["level"], fresh or cooled, fresh or cooled))
            if fresh or cooled:
                notify.send(f"{'■' if i['level']=='crit' else '▲'} {i['msg']}",
                            title="agent-os watchdog", priority=PRIO[i["level"]], tags="rotating_light")
                paged.append(i["msg"])
        # recoveries: anything previously tracked that's no longer an issue
        recovered = [s for s in known if s not in now_sigs]
        for s in recovered:
            cur.execute("DELETE FROM watchdog_alerts WHERE signature=%s", (s,))
            if known[s] is not None:   # only announce things we actually paged about
                notify.send(f"✓ recovered: {s}", title="agent-os watchdog", tags="white_check_mark")
        c.commit()
    beat("watchdog")
    return {"issues": len(issues), "paged": paged, "recovered": len(recovered)}
